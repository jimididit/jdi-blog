...
faq:
  - question: "What is the difference between `merge` and `rebase`?"
    answer: "Merge keeps all commit histories visible, while rebase rewrites commit history for a cleaner linear flow."
  - question: "How can I undo a commit?"
    answer: "Use `git reset` with `--soft` to keep changes or `--hard` to discard them entirely."
  - question: "What is the purpose of staging changes?"
    answer: "Staging (`git add`) lets you select specific changes to include in your next commit, offering control over what gets committed."
  - question: "How do I resolve conflicts during a merge?"
    answer: "Edit conflicting files, stage them with `git add`, and finish with `git commit`."
  - question: "What is a remote repository?"
    answer: "A remote repository is an online storage location for your code, like GitHub, enabling collaboration and backup."
  - question: "Why use branches in Git?"
    answer: "Branches allow you to work on features or fixes independently, keeping the main codebase stable."
---
